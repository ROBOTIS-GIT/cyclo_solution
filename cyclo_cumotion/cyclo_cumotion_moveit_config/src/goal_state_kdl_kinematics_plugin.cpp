...
public:
  ...
  {
    const bool solved = KDLKinematicsPlugin::getPositionIK(pose, seed, solution, error_code,
        options);
    if (solved) {
  ...
  bool searchPositionIK(
    ...
    const kinematics::KinematicsQueryOptions & options = {}) const override
  {
    const bool solved = KDLKinematicsPlugin::searchPositionIK(pose, seed, timeout, solution,
        error_code, options);
    if (solved) {
      updateGoalSpheres(solution);
    }
...
PLUGINLIB_EXPORT_CLASS(cyclo_cumotion_moveit_config::GoalStateKDLKinematicsPlugin,
