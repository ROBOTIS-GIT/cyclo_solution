...
public:
                  ...
                  const std::vector<std::string>& /*loader_tip_frames*/, double search_discretization) override
  {
    ...
    RCLCPP_INFO(node_->get_logger(),
                ...
                ik_link_names_[0].c_str(), ik_link_names_[1].c_str(), joint_model_group_->getVariableCount());
    return joint_names_.size() == 15;
  }

  bool supportsGroup(const moveit::core::JointModelGroup* group, std::string* error_text = nullptr) const override
  {
    const bool supported = group && group->getName() == "wholebody" && group->getVariableCount() == 15;
  ...
  bool searchPositionIK(const geometry_msgs::msg::Pose& pose, const std::vector<double>& seed, double timeout,
                        ...
                        const kinematics::KinematicsQueryOptions& options = {}) const override
  {
    return solve({ pose }, seed, timeout, consistency_limits, solution, {}, error_code, options, nullptr);
  }
  ...
  bool searchPositionIK(const geometry_msgs::msg::Pose& pose, const std::vector<double>& seed, double timeout,
                        ...
                        const kinematics::KinematicsQueryOptions& options = {}) const override
  {
    return solve({ pose }, seed, timeout, consistency_limits, solution, callback, error_code, options, nullptr);
  }

  bool searchPositionIK(const std::vector<geometry_msgs::msg::Pose>& poses, const std::vector<double>& seed,
  ...
  {
    return solve(poses, seed, timeout, consistency_limits, solution, callback, error_code, options, context_state);
  }

  bool getPositionFK(const std::vector<std::string>& links, const std::vector<double>& joint_values,
...
private:
  bool solve(const std::vector<geometry_msgs::msg::Pose>& poses, const std::vector<double>& seed, double timeout,
             const std::vector<double>& consistency_limits, std::vector<double>& solution,
             const IKCallbackFn& callback, moveit_msgs::msg::MoveItErrorCodes& error_code,
             const kinematics::KinematicsQueryOptions& options,
             const moveit::core::RobotState* context_state) const
  {
    ...
    if (!robot_model_ || !joint_model_group_ || seed.size() != joint_model_group_->getVariableCount() ||
    ...
    for (double offset = lift_step;
         seed_lift + offset <= lift_bounds.max_position_ || seed_lift - offset >= lift_bounds.min_position_;
         offset += lift_step)
    {
    ...
    {
      if (std::chrono::duration<double>(std::chrono::steady_clock::now() - started).count() > available_time)
        break;
      ...
      for (std::size_t tip_index = 0; tip_index < poses.size(); ++tip_index)
      {
        ...
        if (remaining <= 0.0 ||
            !candidate.setFromIK(arm_groups_[tip_index], targets[tip_index], ik_link_names_[tip_index],
      ...
      if (error_code.val == moveit_msgs::msg::MoveItErrorCodes::SUCCESS)
      {
        ...
        RCLCPP_DEBUG(node_->get_logger(), "Wholebody goal IK solved with lift %.4f (seed %.4f)", lift, seed_lift);
        return true;
      }
    }
    RCLCPP_WARN(node_->get_logger(), "Wholebody goal IK found no solution across lift range [%.3f, %.3f]",
...
PLUGINLIB_EXPORT_CLASS(cyclo_cumotion_moveit_config::WholebodyGoalKinematicsPlugin, kinematics::KinematicsBase)
