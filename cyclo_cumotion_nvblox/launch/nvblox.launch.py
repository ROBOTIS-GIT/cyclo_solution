...
def launch_setup(context):
    ...
    selected_names = (
        ['right'] if camera_set == 'right' else ['head', 'left', 'right']
    )
    ...
    # ComposableNodeContainer's built-in descriptions are loaded only on the
    # first execution. A respawn creates an empty container, so explicitly
    # reload the segmenter(s) on every process start, including respawns.
...
def generate_launch_description():
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            'camera_set', default_value='all', choices=['all', 'right'],
            description='Use all depth cameras, or only the right wrist camera'),
        DeclareLaunchArgument(
            'robot_description_service_name',
