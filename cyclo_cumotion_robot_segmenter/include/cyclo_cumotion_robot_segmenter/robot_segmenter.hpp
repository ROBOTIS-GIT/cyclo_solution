...
#ifndef CYCLO_CUMOTION_ROBOT_SEGMENTER__ROBOT_SEGMENTER_HPP_
#define CYCLO_CUMOTION_ROBOT_SEGMENTER__ROBOT_SEGMENTER_HPP_
...
private:
  ...
  // Depth frames wait here until the transform at their exact timestamp arrives.
  std::deque<PendingDepthFrame> pending_depth_frames_;
  rclcpp::TimerBase::SharedPtr pending_depth_timer_;
  std::chrono::milliseconds tf_lookup_timeout_{200};
  std::chrono::milliseconds tf_poll_period_{5};
  size_t pending_depth_queue_size_{10};
  ...
  // Robot segmenter joint configuration
  std::vector<std::string> ordered_joint_names_;
  std::vector<std::pair<double, double>> ordered_joint_limits_;
  ...
  // Acknowledges that this segmenter has loaded a specific reload generation.
  rclcpp::Publisher<std_msgs::msg::String>::SharedPtr reload_ack_pub_;
  ...
  void ProcessPendingDepthFrames();
  void CacheRobotCspaceMetadata();
  ...
  // Refresh a moving camera pose at the depth-image timestamp. Returns false
  // when no time-consistent transform is available; callers must drop that
  // frame instead of segmenting with a stale pose.
...
#endif  // CYCLO_CUMOTION_ROBOT_SEGMENTER__ROBOT_SEGMENTER_HPP_
